...
### MMS inference precision

`STT_MMS_DTYPE` selects `float32` (default), `float16`, or `bfloat16` for MMS
on CUDA. Reduced-precision weights are loaded before GPU transfer to avoid a
float32 allocation spike, and audio inputs are cast to match. CPU uses float32.
`STT_COMPUTE_TYPE` continues to control Whisper only. The browser demo explicitly
selects CUDA/float16 through its independent DEMO_STT_DEVICE/DEMO_STT_DTYPE settings.
